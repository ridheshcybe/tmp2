...
Body `{"throttle": 0.0–1.0}` (validated). Overrides the mission profile's throttle from the next tick. **400** if not running. Response: `{"throttle": ..., "status": "set"}`.

### `POST /api/simulation/altitude`
Body `{"altitude_ft": 0–45000}` (validated). Same override semantics. **400** if not running.
