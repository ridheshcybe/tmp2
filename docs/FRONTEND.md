...
Flow: desktop renders a QR (small URL, scan-friendly) → phone opens `phone.html` over HTTPS → camera hand-tracking counts extended fingers → WebRTC offer/answer exchanged → phone drives the cutaway camera (finger count → camera mode). The desktop side lives in the pairing section of `cutaway.html`; signalling details are inline there.

## 5. Dashboard ↔ simulator bridge (iframe postMessage)

The dashboard embeds `cutaway.html` in `#cutawayFrame`:

- **RPM presets**: `index.html` posts `{type: "CUTAWAY_SET_RPM", rpm}` on preset clicks (Idle 1.2K / Cruise 2.4K / Takeoff 5.8K) and re-pushes the current RPM when the iframe reloads. The simulator maps `rpm / 3600` onto its `#speed` slider (auto-widening the slider max for Takeoff) and dispatches a real `input` event so all downstream readouts update.
