...
The backend simulator runs this at `SIM_HZ=10`; mission `duration_s` caps wall-clock run time. Interactive overrides (`POST /api/simulation/throttle|altitude`) beat the profile from the next tick.

## 4. `telemetry_schema.py` — the data contract

- Enums: `FaultType` (9 values incl. HEALTHY), `MissionPhase` (7 phases), `PHASE_ORDER`.
