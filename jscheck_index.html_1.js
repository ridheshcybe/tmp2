    ...
    const DEFAULT_CAPTION =
      'Scan this code with your phone camera to pair.<br>' +
      'Works on the same Wi-Fi - or anywhere, when the ngrok tunnel is up.';
    ...
    if (API) {
      ...
      /*  REST fallback: the phone page polls /__ctl for queued commands
          (works even without a WebRTC data channel).  The ngrok header
          does nothing locally; it only matters if this page itself is
          ever opened through the tunnel. */
      setInterval(async () => {
        try {
          const r = await fetch('/__ctl?since=' + ctlCursor,
