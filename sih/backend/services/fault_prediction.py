...
class FaultPredictionService:
    ...
    def get_all_fault_types(self) -> List[Dict[str, Any]]:
        ...
        Only the injectable fault classes are advertised - UNKNOWN is an
        internal classifier fallback, not something a client can inject,
        and HEALTHY is the absence of a fault.
        """
        return [
            {"type": ft, "criticality": CRITICALITY.get(ft, 0.5),
             "action": ACTIONS.get(ft, "Inspect.")}
            for ft in CRITICALITY if ft not in ("HEALTHY", "UNKNOWN")
        ]
