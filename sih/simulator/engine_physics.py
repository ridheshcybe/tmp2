...
CHT_HEAT_COEFF_KW_PER_RPM = 0.0092  # combustion heat to one head [kW per RPM]
HEAT_TRANSFER_COEFF = 0.067   # cooling coeff at idle [kW/(s·°C·ΔT)]
HEAT_TRANSFER_COEFF_WOT = 0.11  # cooling coeff at redline (airspeed) [kW/(s·°C·ΔT)]
OIL_HEAT_CAPACITY = 8.0      # oil sump thermal inertia [kJ/°C]
OIL_COOLING_COEFF = 0.05     # oil-to-ambient heat rejection [kJ/(s·°C)]
OIL_TEMP_RISE_IDLE_C = 60.0  # steady oil temp above ambient at idle [°C]
OIL_TEMP_RISE_MAX_C = 90.0   # steady oil temp above ambient at redline [°C]
...
THERMAL_EFF = 0.32           # brake thermal efficiency [-]
AIR_GAS_CONSTANT = 287.05    # dry air [J/(kg·K)]
...
VIB_RPM_COEFF = 2.0e-7       # vibration scaling factor [g/RPM²]
#   0.15 + 2.0e-7 · 2700² ≈ 1.6 g at redline - physically plausible for a
#   piston engine (the old 3.5e-5 produced an impossible ~180 g).
...
NOISE_OIL_T_SD = 0.3
NOISE_VIB_SD = 0.005
NOISE_RPM_PROCESS_SD = 0.8   # cycle-to-cycle combustion variability [RPM]
...
class AeroPistonEngine:
    ...
    def _update_rpm(self) -> None:
        ...
        # Combustion cycle-to-cycle variability perturbs the *state*, not
        # just the reported frame - otherwise enable_noise=True engines are
        # byte-identical run to run.
        if self.enable_noise:
            self.rpm += random.gauss(0.0, NOISE_RPM_PROCESS_SD)
    ...
    def _update_fuel_flow(self) -> None:
        """
        Airflow-based fuel flow (the physically consistent route).

        The engine ingests air proportional to MAP, displacement and
        shaft speed (half the revs on a 4-stroke), multiplied by
        volumetric efficiency.  Fuel follows from the mixture AFR,
        which richens with throttle; brake power follows from the
        fuel energy and thermal efficiency.  The previous BMEP chain
        silently dropped a factor of ~20 and reported ~2 L/h at full
        throttle (a real O-320-class engine burns 25-35 L/h there).
        """
        ...
        air_kg_s = (
            (self.map_kpa * 1000.0)
            / (AIR_GAS_CONSTANT * (self.ambient_temp_c + 273.15))
            * displacement_m3
            * (rev_s / 2.0)
            * vol_eff
        )

        # Mixture richens with throttle (λ⁻¹ = equivalence ratio)
        afr = STOICH_AFR * (1.2 - 0.35 * self.throttle)
        afr = max(afr, 8.0)
        self.equivalence_ratio = STOICH_AFR / afr

        # Fuel mass flow [kg/s] → L/hr
    ...
    def _update_cht(self) -> None:
        ...
        Q_comb grows with firing rate (∝ RPM); the cooling coefficient
        also grows with RPM because airflow past the cylinders rises
        with speed.  Together they hold steady-state CHT near ~90 °C at
        idle, ~205 °C at cruise and ~240 °C at redline (the old model
        ran away to 760 °C).
        """
        rpm_ratio = self.rpm / MAX_RPM
        cooling_coeff = (
            HEAT_TRANSFER_COEFF
            + (HEAT_TRANSFER_COEFF_WOT - HEAT_TRANSFER_COEFF) * rpm_ratio
        )
        q_comb_kw = CHT_HEAT_COEFF_KW_PER_RPM * self.rpm
        for i in range(NUM_CYLINDERS):
            q = q_comb_kw + _CHT_OFFSETS[i] * 0.05
            cooling = cooling_coeff * (self.cht_c[i] - self.ambient_temp_c)
            dch_dt = (q - cooling) / CYLINDER_THERMAL_MASS
    ...
    def _update_oil(self) -> None:
        ...
        # Oil temperature: Newton cooling toward a heat-input steady state
        # that RISES with power (60 °C above ambient at idle, 90 °C at
        # redline).  The old quadratic-with-1/cooling form targeted below-
        # ambient temperatures, so oil *cooled* as the throttle opened.
        rpm_ratio = self.rpm / MAX_RPM
        t_steady = (
            self.ambient_temp_c
            + OIL_TEMP_RISE_IDLE_C
            + (OIL_TEMP_RISE_MAX_C - OIL_TEMP_RISE_IDLE_C) * rpm_ratio
        )
