...
#: Compressed "test flight" plan for the frontend demo button: the same
#: STARTUP → TAKEOFF → CLIMB → CRUISE → DESCENT → LANDING shape as the
#: default plan, squeezed into ~2.5 minutes so a whole flight fits in a
#: dashboard session. LANDING ends at idle on the ground, where it holds.
TEST_FLIGHT_PLAN: List[PhaseSegment] = [
    PhaseSegment(MissionPhase.STARTUP, 15.0, 0.10, 0.12, 0.0, 0.0),
    PhaseSegment(MissionPhase.TAKEOFF, 12.0, 0.30, 0.95, 0.0, 800.0),
    PhaseSegment(MissionPhase.CLIMB, 30.0, 0.88, 0.85, 800.0, 18000.0),
    PhaseSegment(MissionPhase.CRUISE, 40.0, 0.78, 0.78, 18000.0, 18000.0),
    PhaseSegment(MissionPhase.DESCENT, 30.0, 0.30, 0.25, 18000.0, 1500.0),
    PhaseSegment(MissionPhase.LANDING, 15.0, 0.15, 0.06, 1500.0, 0.0),
]


#: Interactive "manual" plan: the engine sits parked at ground idle on the
#: runway indefinitely (the plan simply loops by exceeding its duration and
#: holding the last segment). Presets, the throttle slider and the phone
#: controller override the throttle from the next tick; "To Idle" releases
#: the override and parks the engine back here.
MANUAL_PLAN: List[PhaseSegment] = [
    PhaseSegment(MissionPhase.MANUAL, 3600.0, 0.05, 0.05, 0.0, 0.0),
]


def build_test_flight_plan(
    duration_overrides: Optional[Dict[str, float]] = None,
) -> List[PhaseSegment]:
    """
    Return the compressed test-flight plan with optional per-phase duration
    overrides, e.g. ``{"CRUISE": 120.0}``.
    """
    overrides = duration_overrides or {}
    plan: List[PhaseSegment] = []
    for seg in TEST_FLIGHT_PLAN:
...
def build_mission_plan(
    ...
    """
    Return a mission plan with optional per-phase duration overrides,
    e.g. ``{"CRUISE": 3600.0, "ENDURANCE": 4800.0}``.

    ``profile="test_flight"`` selects the compressed ~2.5-minute
    test-flight plan used by the frontend's Test Flight demo.
    ``profile="manual"`` selects the idle-parked interactive plan.
    """
    overrides = duration_overrides or {}
    base = (
        TEST_FLIGHT_PLAN if profile == "test_flight"
        else MANUAL_PLAN if profile == "manual"
        else DEFAULT_PLAN
    )
    plan: List[PhaseSegment] = []
    for seg in base:
