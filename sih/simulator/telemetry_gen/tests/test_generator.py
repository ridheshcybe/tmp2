...
def test_test_flight_plan_shape():
    """Compressed demo plan: same phases in order, ~2.5 min, lands at idle."""
    plan = build_test_flight_plan()
    assert [seg.phase for seg in plan] == [
        MissionPhase.STARTUP,
    ...
    total = sum(seg.duration_s for seg in plan)
    assert 120.0 <= total <= 180.0, f"test flight lasts {total}s"

    profile = MissionProfile(plan)

    # wheels-up mid-TAKEOFF: high throttle, low altitude
    t_to = profile.phase_start_s(MissionPhase.TAKEOFF) + 8.0
    _, thr, alt = profile.at(t_to)
    assert thr > 0.5
    assert alt < 1000.0

    # mid-CRUISE: high and steady
    t_cr = profile.phase_start_s(MissionPhase.CRUISE) + 20.0
    _, thr, alt = profile.at(t_cr)
    assert 0.6 < thr < 0.95
    assert alt > 15000.0

    # after LANDING: back on the ground at idle
    _, thr, alt = profile.at(profile.total_duration_s + 10.0)
    assert thr <= 0.10
    assert alt < 1.0
